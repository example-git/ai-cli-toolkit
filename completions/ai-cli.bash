...
_ai_cli_fill_tool_data() {
  ...
  case "$tool" in
    claude)
      native_flags="--add-dir --agent --agents --allow-dangerously-skip-permissions --allowedTools --append-system-prompt --bare --betas --brief --chrome --continue --dangerously-skip-permissions --debug --debug-file --disable-slash-commands --disallowedTools --effort --fallback-model --file --fork-session --from-pr --help --ide --include-partial-messages --input-format --json-schema --max-budget-usd --mcp-config --mcp-debug --model --name --no-chrome --no-session-persistence --output-format --permission-mode --plugin-dir --print --replay-user-messages --resume --session-id --setting-sources --settings --strict-mcp-config --system-prompt --tmux --tools --verbose --version --worktree -c -d -h -n -p -r -v -w"
      native_subcommands="auth auto-mode doctor mcp setup-token"
      file_flags="--debug-file --file --json-schema --mcp-config --settings"
      dir_flags="--add-dir --plugin-dir"
      ;;
    codex)
      native_flags="--add-dir --ask-for-approval --cd --config --dangerously-bypass-approvals-and-sandbox --disable --enable --full-auto --help --image --local-provider --model --no-alt-screen --oss --profile --remote --sandbox --search --version -C -V -a -c -h -i -m -p -s"
